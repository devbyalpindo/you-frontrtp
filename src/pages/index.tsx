...
const Home: React.FC<ProductProps> = ({ data }) => {
  ...
  const [baseUrl, setBaseUrl] = useState("https://www.youslot88s.co/");
  ...
  return (
    <div className="bg-background">
      <Head>
        ...
        />
        <meta property="og:url" content="https://bigmantab.top" />
          ...
          <Slider {...settings} className="flex items-center">
            {selector.product.map((item: CompanyTypes) => {
              return (
                ...
                >
                  <Image
                    src={`https://bigmantab.top/${item.images}`}
                    width={100}
                    height={100}
                  />
                </button>
...
export async function getServerSideProps() {
  const response = await fetch("https://bigmantab.top/api/product");
